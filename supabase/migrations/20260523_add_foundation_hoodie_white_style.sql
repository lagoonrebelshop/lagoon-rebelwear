-- 20260523_add_foundation_hoodie_white_style.sql
-- Lagoon Rebel Wear — Add Foundation Hoodie White style
-- Obiettivo: predisporre la variante colore White della Foundation Hoodie.
...
select
  ...
  'White',
  'white',
  'Foundation Purple Logo',
  'standard',
  p.price_cents,
  coalesce(p.currency, 'EUR'),
  'coming_soon',
  'hidden',
  'drop-01-foundation/foundation-hoodie-white',
  10
from public.products p
where p.slug = 'foundation-hoodie'
  and not exists (
    select 1
    from public.product_styles ps
    where ps.product_id = p.id
      and ps.slug = 'foundation-hoodie-white'
  );
